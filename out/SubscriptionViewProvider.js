...
exports.SubscriptionViewProvider = void 0;
class SubscriptionViewProvider {
    ...
    // 订阅信息页面
    _getSubscriptionHtml(activeSubscriptions) {
        // 定义排序顺序
        const planOrder = {
            'FREE': 1,
            'PRO': 2,
        ...
        // 分类订阅：额度>0和额度<=0
        const activeSubscriptionsWithCredit = sortedSubscriptions.filter((sub) => {
            const currentCredits = sub.currentCredits || 0;
            return currentCredits > 0;
        });
        const emptySubscriptions = sortedSubscriptions.filter((sub) => {
            const currentCredits = sub.currentCredits || 0;
            return currentCredits <= 0;
        });
        // 计算总额度
        ...
        // 渲染有额度的订阅
        activeSubscriptionsWithCredit.forEach((sub) => {
        ...
        if (emptySubscriptions.length > 0) {
            ...
            emptySubscriptions.forEach((sub) => {
                const cardHtml = renderSubscriptionCard(sub);
                html += cardHtml;
                // 累加总额度
                const currentCredits = sub.currentCredits || 0;
...
exports.SubscriptionViewProvider = SubscriptionViewProvider;
SubscriptionViewProvider.viewType = 'subscriptionView';
//# sourceMappingURL=SubscriptionViewProvider.js.map