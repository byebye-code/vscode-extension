...
export class AnnouncementNotificationService {
    ...
    public async start() {
        try {
            ...
            // 检查是否已经显示过这个公告
            const lastShownAnnouncementId = this._context.globalState.get<number>('88code_last_shown_announcement_id');
     ...
     * 获取最新公告
     */
    private async fetchLatestAnnouncement(token: string): Promise<any | null> {
        try {
            const response = await this.httpRequestWithAuth(
                'GET',
                'https://88code.org/admin-api/announcement/queryEnabledList',
                token
            );
     ...
     * 显示公告面板
     */
    private showAnnouncementPanel(announcement: any) {
        // 如果已经有面板打开，先关闭
        if (this._panel) {
            this._panel.dispose();
        }
     ...
     * 生成 Webview 内容
     */
    private getWebviewContent(announcement: any): string {
        return `<!DOCTYPE html>
        <html lang="zh">
        <head>
            <meta charset="UTF-8">
