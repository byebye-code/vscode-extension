...
export class LoginViewProvider implements vscode.WebviewViewProvider {
     ...
     * 处理账号密码登录
     */
    private async handleAccountLogin(username: string, password: string, captchaCode: string, captchaUuid: string) {
        try {
            // 验证输入
    ...
    public async autoRelogin(): Promise<boolean> {
        try {
            const username = this._context.globalState.get<string>('88code_username');
            const password = this._context.globalState.get<string>('88code_password');
