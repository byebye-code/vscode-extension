...
export class CodexService {
    ...
    async fetchCodexUsage() {
        try {
            ...
            const response = await this.httpRequestWithAuth(
                'GET', 
                'https://www.88code.org/admin-api/cc-admin/user/openai-daily-usage', 
                token
            );
    ...
    showCachedCodexUsage() {
        const cachedData = this._context.globalState.get('88code_cached_codex') as any;
    ...
    updateStatusBarDisplay(remaining?: number, limit?: number, isCached: boolean = false) {
    ...
    httpRequestWithAuth(method: string, url: string, token: string, data?: any): Promise<any> {
